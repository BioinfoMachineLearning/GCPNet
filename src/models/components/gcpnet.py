...
class GCPEmbedding(nn.Module):
    def __init__(
        ...
        nonlinearities: Tuple[Optional[str]] = (None, None),
        num_lig_flags: int = 2,
        cfg: DictConfig = None,
        pre_norm: bool = True
    ):
...
class GCPMessagePassing(nn.Module):
    ...
    def aggregate(
        ...
        row, col = edge_index
        aggregate = scatter(message, row if self.aggregate_with_row else col, dim=0, dim_size=dim_size, reduce=self.reduce_function)
        return aggregate
...
class GCPInteractions2(nn.Module):
    def __init__(
        self,
        node_dims: ScalarVector,
        edge_dims: ScalarVector,
        ...
        self.interaction = message_function(
            ...
            mp_cfg=layer_cfg.mp_cfg,
            reduce_function=reduce_function,
            use_scalar_message_attention=getattr(layer_cfg, "use_scalar_message_attention", False),
            aggregate_with_row=getattr(layer_cfg, "aggregate_with_row", False)
        )
        ...
        self.gcp_norm = nn.ModuleList([GCPLayerNorm(node_dims)])
        self.gcp_dropout = nn.ModuleList([GCPDropout(dropout)])
        ...
        if updating_node_positions:
            ...
            position_output_dims = (node_dims.scalar, 1)
