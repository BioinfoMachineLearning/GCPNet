...
class GCPNetLBALitModule(LightningModule):
    """LightningModule for ligand binding affinity (LBA) prediction using GCPNet.
    ...
    def __init__(
        ...
        **kwargs
        ...
        training_metrics_to_log = ["RMSE", "PearsonCorrCoef"]
        metrics = {
            "RMSE": partial(torchmetrics.regression.mse.MeanSquaredError, squared=False),
            "PearsonCorrCoef": partial(torchmetrics.PearsonCorrCoef),
            "SpearmanCorrCoef": partial(torchmetrics.SpearmanCorrCoef)
        }
        for phase in phases:
        ...
        self.val_pearson_corr_coef_best = torchmetrics.MaxMetric()
    ...
    def on_train_start(self):
        ...
        self.metrics[self.val_phase]["PearsonCorrCoef"].reset()
        self.val_rmse_best.reset()
        self.val_pearson_corr_coef_best.reset()
    ...
    def validation_epoch_end(self, outputs: List[Any]):
        ...
        self.val_pearson_corr_coef_best.update(self.metrics[self.val_phase]["PearsonCorrCoef"].compute())
        ...
        self.log(
            f"{self.val_phase}/PearsonCorrCoef_best",
            self.val_pearson_corr_coef_best.compute(),
            prog_bar=True,
            sync_dist=True
        )
...
if __name__ == "__main__":
    ...
    cfg = omegaconf.OmegaConf.load(root / "configs" / "model" / "gcpnet_lba.yaml")
